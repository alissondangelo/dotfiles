...
local function factory(args)
    ...
    local function color(fg, bg, text)
    ...
    function cal.build(month, year)
        ...
        local notifytable = { [1] = string.format("%s%s\n", string.rep(" ", floor((28 - this_month:len())/2)), bold(this_month)) }
        ...
        for x = 1,mth_days do
            ...
            if x == today then
                ...
                strx = bold(color(cal.notification_preset.bg, cal.notification_preset.fg, x) .. " ")
            end
