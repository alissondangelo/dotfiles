...
-- Lock,Sleep,Restart,Shutdown -------------------------------------------------
    ...
    awful.key({ modkey }, "i",
        function(c)
            if beautiful.gap_single_client ~= nil then
                beautiful.gap_single_client = not beautiful.gap_single_client
            else
                beautiful.gap_single_client = false
            end
        end,
        { description = "toggle gap when single client", group = "client" }),

    --border------------------------------------------------------------------------
    awful.key({ modkey }, "u",
        function(c)
            if beautiful.border_width > 0 then
                beautiful.b = beautiful.border_width
                beautiful.border_width = 0
            elseif beautiful.b ~= 0 then
                beautiful.border_width = beautiful.b
            end
        end,
        { description = "toggle client border", group = "client" }),
